...
const CustomerPanel = ({ customer, customers, onCustomerChange, onAddCustomer, onRequestDeleteCustomer = () => {}, onPickCustomer, onLookupCustomer }) => {
  return (
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 bg-slate-900 border border-slate-700 text-slate-100 placeholder:text-slate-500 rounded-xl focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 transition-all"
              />
              {onLookupCustomer && (
                <button
                  type="button"
                  onClick={() => customer.vat && onLookupCustomer(customer.vat)}
                  className="px-3 py-2 bg-sky-500/80 hover:bg-sky-500 text-white font-medium rounded-xl transition-all shadow-lg shadow-sky-500/20 text-sm"
                  title="Αναζήτηση στοιχείων πελάτη από GSIS"
                >
                  🔍 Αναζήτηση
                </button>
              )}
