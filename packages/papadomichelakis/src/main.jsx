...
const DEFAULT_SERVICES_DESC = 'Εγκατάσταση - Προμήθεια τέντας/πέργκολας';
const DEFAULT_VILLA_DESC = 'Παροχή υπηρεσιών βραχυχρόνιας μίσθωσης τουριστικού καταλύματος (βίλα)';
const SERVICES_BRANCH_IDS = ['papadomichelakis'];
...
const getDefaultDescriptionForBranch = (branchId) => {
  if (SERVICES_BRANCH_IDS.includes(branchId)) return DEFAULT_SERVICES_DESC;
...
function InvoiceApp() {
  ...
  // State management
  const [branch, setBranch] = useState('papadomichelakis');
  const branchCfg = useMemo(() => BRANCHES[branch], [branch]);
  const [customer, setCustomer] = useState({ name: '', vat: '', email: '', address: '', city: '' });
  // Initial branch is 'papadomichelakis' so prefill default description
  const [items, setItems] = useState([{ description: getDefaultDescriptionForBranch('papadomichelakis'), qty: 1, price: 0, vatRate: 24 }]);
  ...
  const [customers, setCustomers] = useState(() => {
    try { return JSON.parse(localStorage.getItem(storageKeyCustomers('papadomichelakis')) || '[]'); } catch { return []; }
  });
  ...
  // When switching to a services branch, if all item descriptions are empty, set first to default
  useEffect(() => {
    if (!SERVICES_BRANCH_IDS.includes(branch)) return;
    if (!items || !items.length) return;
    const allEmpty = items.every(it => !String(it.description || '').trim());
    if (allEmpty) {
      const clone = [...items];
      clone[0] = { ...clone[0], description: DEFAULT_SERVICES_DESC };
      setItems(clone);
    }
  }, [branch]);
  ...
  useEffect(() => {
    ...
    const needReplace = items.some(it => String(it.description || '').trim() === DEFAULT_SERVICES_DESC);
    if (needReplace) {
      const replaced = items.map((it, idx) => (String(it.description || '').trim() === DEFAULT_SERVICES_DESC ? { ...it, description: idx === 0 ? DEFAULT_VILLA_DESC : '' } : it));
      setItems(replaced);
    }
  }, [branch]);
  ...
  const resetForm = () => {
    setBranch('papadomichelakis');
  setCustomer({ name: '', vat: '', email: '', address: '', city: '' });
  setItems([{ description: DEFAULT_SERVICES_DESC, qty: 1, price: 0, vatRate: 24 }]);
