...
const HistoryTable = ({ history = [], branches = {}, currentBranchId, dateFormat='DD-MM-YYYY', onPreview, onDownload, onReceipt, onIssueSurcharge, onDeleteEntry, onCancel, disableActions = false }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [branchFilter, setBranchFilter] = useState(currentBranchId || 'all');
  const [statusFilter, setStatusFilter] = useState('all');
  const [fromDate, setFromDate] = useState('');
  const [toDate, setToDate] = useState('');
  const [copiedMark, setCopiedMark] = useState('');

  useEffect(() => {
    if (!copiedMark) return;
    const timeout = setTimeout(() => setCopiedMark(''), 2000);
  ...
  const formatDateDisplay = (value) => formatDate(value, dateFormat);
  ...
  const exportCSV = () => {
    ...
    const rows = filteredRows.map((entry) => {
      const totals = deriveTotals(entry);
      const branchLabel = branches[entry.branchId]?.label || entry.branchId || '';
  const issueDateRaw = entry.issueDate || entry.invoiceDate || '';
  const issueDate = formatDateDisplay(issueDateRaw);
  ...
  const exportPDF = () => {
    ...
    filteredRows.forEach((entry) => {
      ...
      const branchLabel = branches[entry.branchId]?.label || entry.branchId || '';
      body.push([
  { text: formatDateDisplay(entry.issueDate || entry.invoiceDate || ''), fontSize: 9 },
  ...
  return (
    <div className="bg-slate-900/80 rounded-2xl shadow-xl border border-slate-800/60 overflow-hidden backdrop-blur">
      ...
      <div className="p-6 space-y-6">
        ...
        {copiedMark && (
          <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-xs font-medium text-emerald-100">
            MARK {copiedMark} αντιγράφηκε στο clipboard.
          </div>
        )}
                ...
                {filteredRows.map((entry, index) => {
                  ...
                  return (
                    <tr key={key} className="border-b border-slate-800/40 hover:bg-slate-900/60 transition-colors">
                      <td className="py-3 px-3 whitespace-nowrap font-medium text-slate-200">{formatDateDisplay(entry.issueDate || entry.invoiceDate)}</td>
                      ...
                      <td className="py-3 px-3 text-xs text-center font-mono text-slate-400">
                        {entry.mark ? (
                          <button
                            type="button"
                            onClick={() => handleCopyMark(entry.mark)}
                            disabled={disableActions}
                            className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 transition ${
                              copiedMark === entry.mark
                                ? 'border-emerald-500/50 bg-emerald-500/15 text-emerald-200'
                                : 'border-slate-700/60 bg-slate-900/60 text-slate-200 hover:border-emerald-500/40'
                            } disabled:cursor-not-allowed disabled:opacity-50`}
                            title="Αντιγραφή MARK στο clipboard"
                          >
                            <span>{entry.mark}</span>
                            <span className="text-[10px] uppercase tracking-[0.3em]">
                              {copiedMark === entry.mark ? 'COPIED' : 'COPY'}
                            </span>
                          </button>
                        ) : (
                          entry.error || '—'
                      ...
                      <td className="py-3 px-3">
                        <div className="flex flex-wrap justify-center gap-2">
                          ...
                          {onIssueSurcharge && VILLA_BRANCH_IDS.includes(entry.branchId) && (
                            <button
                              type="button"
                          ...
                          {onCancel && entry.status === 'sent' && (
                            <button
                              type="button"
                              onClick={() => onCancel(entry)}
                              disabled={disableActions}
                              className="px-3 py-1 rounded-lg border border-orange-500/30 bg-orange-500/10 text-xs font-medium text-orange-200 transition hover:bg-orange-500/20 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              ❌ Ακύρωση
                            </button>
                          )}
                          {onDeleteEntry && (
                            <button
                              ...
                              disabled={disableActions}
                              className="px-3 py-1 rounded-lg border border-rose-500/30 bg-rose-500/10 text-xs font-medium text-rose-200 transition hover:bg-rose-500/20 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              Διαγραφή
                            </button>
                          )}
